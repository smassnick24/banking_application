...
def define_database():
    cur.execute(
        "CREATE TABLE IF NOT EXISTS accounts (name TEXT, account_number INTEGER, routing_number INTEGER, account_type TEXT, balance FLOAT)")
...
def register_new_bankaccount(account: BankAccount):
    ...
    if len(cur.execute(f"SELECT * FROM accounts WHERE account_number = {account.get_acnt()}").fetchall()) != 0:
        ...
        cur.execute(
            f"INSERT INTO accounts VALUES('{account.get_name()}', '{account.get_acnt()}', '{account.get_route()}', '{account.get_type()}', '{account.get_bal():.2f}')")
        conn.commit()
        return True
...
def update_balance(account: BankAccount):
    ...
    conn.commit()
    balance = list(cur.execute(f"SELECT * FROM accounts WHERE account_number = {account.get_acnt()}").fetchall())[0][
        4]  # grabbing the balance [0] for first account [4] for the balance index of the tuple
    return balance
